...
// Tipo extendido con status derivado
interface ReporteWithDerivedStatus extends ReporteWithDistance {
...
export const ReportesComparativeAnalysis = memo(function ReportesComparativeAnalysis() {
  ...
  // Filtrar reportes no eliminados y agregar status derivado
  const reportesConStatus = useMemo<ReporteWithDerivedStatus[]>(() => {
    return reportes
  ...
  const searchableItems = useMemo<FilterOption[]>(() => {
    return reportesConStatus.map(r => ({
      id: r.id,
      label: r.nombre,
  ...
  return (
    <div className="space-y-6">
      ...
      <EntityAnalysisSkeleton<ReporteWithDerivedStatus>
      data={reportesConStatus}
      dateField="created_at"
      entityName="Reporte"
      entityNamePlural="Reportes"
      statusField="derivedStatus"
