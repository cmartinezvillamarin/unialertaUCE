    ...
    <Card className={cn('border border-border shadow-sm', animationClasses.fadeIn, className)}>
      ...
      <CardContent className="space-y-6">
        {PERMISSION_GROUP_CONFIG.map((group) => {
          ...
          // Si es readOnly, solo mostrar permisos que el usuario tiene asignados
          const visiblePermissions = readOnly 
            ? groupPermissions.filter(p => hasPermission(p) || isLocked(p))
            : groupPermissions;

          // Si no hay permisos visibles en este grupo, no mostrar el grupo
