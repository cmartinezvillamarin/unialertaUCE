...
// Roles que pueden ver los tabs de auditoría
...
export function UserDetails({ user }: UserDetailsProps) {
  ...
  const { userRoles: currentUserRolesData } = useUserDataReady();
  // Solo pasar ownerId para visibilidad RLS, pero canEdit solo por permiso
  const { canEdit } = useEntityPermissions({ 
    entityKey: 'usuarios',
    ownerId: user.id  // El user.id es el profile.id
  });
  ...
  // Verificar si puede ver los tabs de auditoría
  const canViewAudit = useMemo(() => {
    if (!currentUserRolesData) return false;
    return AUDIT_ROLES.some(role => hasRole(currentUserRolesData, role));
  }, [currentUserRolesData]);
  ...
  const createdReportes = useMemo(() => {
    return reportes.filter(r => r.user_id === user.id);
  }, [reportes, user.id]);

  // Convertir reportes asignados a EntityListItem
  ...
  // Convertir reportes creados a EntityListItem
  const createdReportesListItems: EntityListItem[] = useMemo(() => {
    return createdReportes.map(r => ({
      id: r.id,
      nombre: r.nombre,
      descripcion: r.descripcion,
      status: r.status,
  ...
  // Switch actions para estado del usuario - solo visible si tiene permiso
  const switchActions: EntitySwitchAction[] = canEdit ? [
    {
  ...
  // Action buttons - solo visible si tiene permiso
  ...
  const tabs = [
    ...
    {
      value: 'reportes-creados',
      label: 'Reportes Creados',
      icon: FileText,
      badge: createdReportes.length,
      content: (
        <EntityListCard
          title={`Reportes Creados (${createdReportes.length})`}
          subtitle={`Reportes creados por ${user.name || user.username}`}
          items={createdReportesListItems}
          detailRoute="/reportes"
          emptyMessage="Sin reportes creados"
          emptySubMessage="Este usuario no ha creado reportes"
    ...
    },
    {
      ...
      content: (
        <UserRolesManager 
          userId={user.id} 
          readOnly={!canEdit}  // Solo admins pueden editar roles, no el propio usuario
        />
      ),
    },
    {
      ...
      icon: Key,
      content: (
        <UserPermissionsManager userId={user.id} readOnly={!canEdit} />
      ),
    },
    {
      value: 'auditoria',
  ...
  ].filter(tab => !['auditoria', 'cambios', 'actividad'].includes(tab.value) || canViewAudit);
