...
import { useFriendRequestNotifications } from "@/hooks/controlador";
...
function LayoutContent() {
  ...
  const { loading: authLoading, isAuthenticated, hasInitialized, isSigningOut, user } = useAuth();
  const { isReady, isLoading: dataLoading } = useUserDataReady();
  
  // Inicializar presencia global cuando el usuario está autenticado
  useUserPresence();
  
  // Escuchar notificaciones push de solicitudes de amistad
  useFriendRequestNotifications({ userId: user?.id, enabled: isAuthenticated });
