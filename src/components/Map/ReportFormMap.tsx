...
interface ReportFormMapProps {
  ...
  className?: string;
  /** Si es true, deshabilita selección/arrastre de ubicación */
  readOnly?: boolean;
}
...
export const ReportFormMap: React.FC<ReportFormMapProps> = ({
  ...
  // Hook de Overpass API - usar ubicación del reporte seleccionado
  const {
    nodes,
    areas,
  ...
  // Hook para obtener datos de ubicación OSM automáticamente
  ...
  useEffect(() => {
    ...
    // Click handler for selecting location - optimizado para respuesta inmediata
    const onClick = (e: L.LeafletMouseEvent) => {
      if (readOnlyRef.current) return;
      const { lat, lng } = e.latlng;
      
      // INMEDIATAMENTE mostrar el marcador con coordenadas como placeholder
      // Campos OSM vacíos por defecto - se limpian automáticamente
      const immediateLocation: LocationData = {
        latitude: lat,
        longitude: lng,
        address: `${lat.toFixed(6)}, ${lng.toFixed(6)}`,
        // Campos OSM explícitamente undefined para indicar que aún no hay datos
      ...
      const osmPromise = fetchOSMLocationDataRef.current(lat, lng).catch(() => ({}));
      
      // Actualizar con los datos que lleguen - SOLO si existen valores válidos
      Promise.all([nominatimPromise, osmPromise]).then(([nominatimData, osmData]) => {
        const address = nominatimData?.display_name?.trim() || `${lat.toFixed(6)}, ${lng.toFixed(6)}`;
        const osmResult = osmData as { edificio?: string; piso?: string; aulaSala?: string; puntoReferencia?: string };
        
        // Solo incluir campos OSM si tienen valores reales (no strings vacíos)
        onLocationSelect({
          latitude: lat,
          longitude: lng,
          address,
          // Si no hay valor OSM, mantener undefined para que el campo quede en blanco
  ...
  // Cargar áreas OSM cuando la ubicación del reporte cambia y OSM está visible
  useEffect(() => {
    if (osmVisible && selectedLocation) {
      fetchAreas(selectedLocation.latitude, selectedLocation.longitude, 500);
    } else if (osmVisible && location) {
      // Fallback a ubicación del usuario si no hay reporte seleccionado
      fetchAreas(location.latitude, location.longitude, 500);
    }
  }, [osmVisible, selectedLocation?.latitude, selectedLocation?.longitude, location?.latitude, location?.longitude, fetchAreas]);

  // Ref para almacenar onLocationSelect y fetchOSMLocationData y evitar recrear efectos
  const onLocationSelectRef = useRef(onLocationSelect);
  onLocationSelectRef.current = onLocationSelect;
  
  const fetchOSMLocationDataRef = useRef(fetchOSMLocationData);
  fetchOSMLocationDataRef.current = fetchOSMLocationData;
  ...
  useEffect(() => {
    ...
    if (selectedLocation?.latitude && selectedLocation?.longitude) {
      ...
      try {
        ...
        // Handle drag end to update location - optimizado para respuesta inmediata
        if (!readOnlyRef.current) marker.on('dragend', () => {
          const { lat, lng } = marker.getLatLng();
          
          // INMEDIATAMENTE actualizar con coordenadas y LIMPIAR campos OSM
          onLocationSelectRef.current({
            latitude: lat,
            longitude: lng,
            address: `${lat.toFixed(6)}, ${lng.toFixed(6)}`,
            // Campos OSM explícitamente undefined para limpiar los campos
            osmEdificio: undefined,
            osmPiso: undefined,
            osmAulaSala: undefined,
            osmPuntoReferencia: undefined,
  ...
  }, [selectedLocation, location, fetchAreas]);
