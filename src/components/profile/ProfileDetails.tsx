...
import { hasRole } from '@/hooks/entidades';
...
export function ProfileDetails() {
  ...
  const { profile, userRoles, isLoading, isReady, hasPermission } = useUserDataReady();
  ...
  // Verificar si puede ver el tab de Reportes (necesita permiso editar_reporte)
  const canViewReportes = useMemo(() => {
    return hasPermission('editar_reporte');
  }, [hasPermission]);
      ...
      <Card className={cn("overflow-hidden", transitionClasses.card)}>
        <CardContent className="p-0">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            {/* Tab Content: Reportes - Solo visible si tiene permiso */}
            {canViewReportes && (
            ...
            {/* Tab Content: Actividad - Solo visible para roles autorizados */}
            {canViewAudit && (
            ...
            {/* Tab Content: Cambios - Solo visible para roles autorizados */}
            {canViewAudit && (
            ...
            friends.map((friend) => {
              ...
              return (
                <Card 
                  ...
                  onClick={() => navigate(friendProfile.username ? `/perfil/${friendProfile.username}` : `/perfil/id/${friendProfile.id}`)}
                >
