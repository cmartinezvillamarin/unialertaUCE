...
export function UserPresenceProvider({ children }: UserPresenceProviderProps) {
  ...
  useEffect(() => {
    ...
    const initializePresence = async (authUserId: string) => {
      ...
      // Manejar cuando un usuario se va - limpiar su ubicación
        ...
        setOnlineUsers((prev) => {
          ...
          leftPresences.forEach((presence) => {
            ...
            if (p.profile_id) {
              ...
              // Limpiar ubicación del usuario que se desconectó (solo si no somos nosotros)
              if (currentUserRef.current && p.profile_id !== currentUserRef.current.profileId) {
                console.log('[UserPresence] User left, cleaning their location:', p.profile_id);
                supabase.functions.invoke('cleanup-user-locations', {
                  body: { action: 'cleanup_user', user_id: p.profile_id }
                }).catch(err => console.warn('[UserPresence] Error cleaning location:', err));
              }
            }
          });
          return updated;
