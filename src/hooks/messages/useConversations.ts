...
export function useConversations(options: UseConversationsOptions = {}): UseConversationsReturn {
  ...
  // Obtener profile_id desde el cache de React Query (precargado por useUserDataReady)
  const cachedProfile = queryClient.getQueryData<{ id: string }>(['profile', undefined]) as { id: string } | undefined;
  
  // Intentar obtener de cualquier key de profile en el cache
  const [currentProfileId, setCurrentProfileId] = useState<string | null>(null);
  
  useEffect(() => {
    const getProfileFromCache = async () => {
      // Primero intentar desde el cache
      const queries = queryClient.getQueriesData<{ id: string }>({ queryKey: ['profile'] });
      ...
      // Fallback: obtener desde auth si no hay en cache
