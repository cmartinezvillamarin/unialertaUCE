...
// Key para detectar recarga de página
const PAGE_RELOAD_KEY = 'page_reload_timestamp';

/**
 * Detecta si la página fue recargada y limpia cachés si es necesario
 */
function handlePageReload(): void {
  ...
  if (isReload) {
    // Limpiar todos los cachés de entidades
    const keysToRemove: string[] = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key?.startsWith('entity_cache:')) {
        keysToRemove.push(key);
      }
    }
    keysToRemove.forEach(key => localStorage.removeItem(key));
    console.log('[Cache] Page reload detected, cleared entity caches');
  }
}

// Ejecutar una sola vez al cargar el módulo
handlePageReload();
