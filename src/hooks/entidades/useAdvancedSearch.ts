...
export function useInlineDetector(text: string, options: UseInlineDetectorOptions = {}) {
  ...
  const { data: mentionSuggestions } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Si hay query, filtrar por nombre o username
      if (mentionQuery && mentionQuery.length >= 1) {
        query = query.or(`name.ilike.%${mentionQuery}%,username.ilike.%${mentionQuery}%`);
      }
      ...
      const { data } = await query.order('name').limit(5);
      return data || [];
    },
    enabled: enabled && activeInput?.type === 'mention',
    staleTime: 30000,
  });
