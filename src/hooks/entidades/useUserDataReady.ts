...
export function useUserDataReady(): UserDataState {
  ...
  const [state, setState] = useState<Omit<UserDataState, 'hasRole' | 'hasPermission'>>({
    isReady: false,
    isLoading: true,
    userId: null,
    profile: null,
  ...
  const hasRole = useCallback((role: UserRole): boolean => {
    return state.userRoles?.roles?.includes(role) ?? false;
  }, [state.userRoles]);

  const hasPermission = useCallback((permission: UserPermission): boolean => {
    return state.userRoles?.permisos?.includes(permission) ?? false;
  }, [state.userRoles]);
