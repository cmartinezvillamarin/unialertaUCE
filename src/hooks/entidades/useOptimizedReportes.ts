...
export function useOptimizedReportes() {
    ...
    mutationFn: async (id: string) => {
      const { error } = await supabase
        ...
        .update({ 
          ...
          activo: false,
          status: 'eliminado', // Estado explícito al eliminar
  ...
  // Toggle status - al desactivar, también desasignar y marcar como resuelto
  const toggleStatus = async (id: string, currentStatus: boolean) => {
    const updates: ReporteUpdate = { 
      activo: !currentStatus,
      updated_at: new Date().toISOString(),
    };
    
    // Si se desactiva, desasignar y marcar como resuelto
    if (currentStatus) {
