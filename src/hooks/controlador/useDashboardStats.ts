...
import { getDerivedReportStatus } from '@/lib/reportStatus';
...
export function useDashboardStats(): UseDashboardStatsReturn {
  ...
  // Calcular estadísticas desde datos reales de reportes usando status derivado
  const stats = useMemo<DashboardStats | null>(() => {
    if (reportesLoading || usuariosLoading) return null;

    // Filtrar solo reportes no eliminados para las estadísticas principales
    const reportesNoEliminados = reportes.filter(r => !r.deleted_at);
    const usuariosActivos = usuarios.filter(u => !u.deleted_at);

    // Calcular status derivado para cada reporte
    const statusCounts = { pendiente: 0, en_progreso: 0, resuelto: 0, rechazado: 0, cancelado: 0 };
    reportesNoEliminados.forEach(r => {
      const derivedStatus = getDerivedReportStatus({
        activo: r.activo,
        assigned_to: r.assigned_to,
        deleted_at: r.deleted_at,
        status: r.status,
      });
      if (derivedStatus !== 'eliminado' && statusCounts[derivedStatus] !== undefined) {
        statusCounts[derivedStatus]++;
      }
    });

    return {
      totalReportes: reportesNoEliminados.length,
      ...
      reportesPendientes: statusCounts.pendiente,
      reportesEnProceso: statusCounts.en_progreso,
      reportesResueltos: statusCounts.resuelto,
    };
