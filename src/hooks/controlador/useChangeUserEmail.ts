...
export function useChangeUserEmail(options: UseChangeUserEmailOptions = {}) {
  ...
  const changeUserEmail = useCallback(async (
    ...
    try {
      // Guardar sesión actual (admin) para evitar que signUp cambie la sesión del navegador.
      const { data: { session: sessionToRestore } } = await supabase.auth.getSession();

      const restoreSessionIfNeeded = async () => {
        if (!sessionToRestore?.access_token || !sessionToRestore.refresh_token) return;

        const { data: { session: currentSession } } = await supabase.auth.getSession();
        if (currentSession?.user?.id === sessionToRestore.user?.id) return;
      ...
      const normalizeSignUpError = (message: string) => {
        if (message.includes('Error sending confirmation email')) {
          return 'Supabase no pudo enviar el correo de confirmación (SMTP). Revisa Authentication → Email (SMTP) en Supabase.';
        }
        return message;
      };
      ...
      // Asegurar que la sesión del admin se mantenga intacta.
      ...
      if (signUpError) {
        ...
        throw new Error(`Error al crear nuevo usuario: ${normalizeSignUpError(signUpError.message)}. El perfil quedó desvinculado.`);
      }
