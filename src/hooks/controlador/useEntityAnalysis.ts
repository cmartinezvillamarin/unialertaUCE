...
export function useEntityAnalysis<T extends Record<string, unknown>>(
  ...
  const stats = useMemo<EntityStats>(() => {
    ...
    filteredData.forEach((item) => {
      // Contar por estado (soporta ambos formatos: en_progreso y en_proceso)
      if (statusField) {
        const status = String(item[statusField]);
        if (status === 'pendiente') pending++;
        else if (status === 'en_progreso' || status === 'en_proceso') inProgress++;
  ...
  const priorityDistribution = useMemo(() => {
    ...
    filteredData.forEach((item) => {
      ...
      // Clasificar por estado (soporta español e inglés)
