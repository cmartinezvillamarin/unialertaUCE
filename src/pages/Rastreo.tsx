...
// Tipo extendido para incluir datos de categoría/tipo para el heatmap
interface ReportForHeatmap extends ReportForNavigation {
...
const Rastreo: React.FC = () => {
  ...
  const [filteredMapCount, setFilteredMapCount] = useState<number>(0);
  ...
  // Preparar datos de categorías para el heatmap
  const categoriesForHeatmap = useMemo(() => {
    return categorias
  ...
  // Preparar datos de tipos para el heatmap (incluye category_id)
  const tiposForHeatmap = useMemo(() => {
    return tipoReportes
  ...
  // Convertir reportes de la base de datos al formato de navegación con datos de heatmap
  const reportsForNavigation = useMemo<ReportForHeatmap[]>(() => {
    ...
    return reportes
      ...
      .map(r => {
        ...
        return {
          ...
          // Datos para heatmap
          categoryId: r.categories?.id || r.categoria_id,
          categoryColor: r.categories?.color || null,
          categoryName: r.categories?.nombre || null,
          tipoId: r.tipo_categories?.id || r.tipo_reporte_id,
          tipoColor: r.tipo_categories?.color || null,
          tipoName: r.tipo_categories?.nombre || null,
        };
      });
  }, [reportes, filter]);

  // Reportes con distancia para el mapa (combinar datos de navegación con distancia)
  const reportsWithHeatmapData = useMemo(() => {
    // Crear un mapa de los datos de heatmap por ID
    const heatmapDataById = new Map(
      reportsForNavigation.map(r => [r.id, r])
    );

    // Combinar con allReportsWithDistance
    return allReportsWithDistance.map(r => {
      const heatmapData = heatmapDataById.get(r.id);
      return {
        ...r,
        categoryId: heatmapData?.categoryId,
        categoryColor: heatmapData?.categoryColor,
        categoryName: heatmapData?.categoryName,
        tipoId: heatmapData?.tipoId,
        tipoColor: heatmapData?.tipoColor,
        tipoName: heatmapData?.tipoName,
      };
    });
  }, [allReportsWithDistance, reportsForNavigation]);
  ...
  // Inicializar el contador con el total de reportes
  useEffect(() => {
    if (reportsWithHeatmapData.length > 0 && filteredMapCount === 0) {
      setFilteredMapCount(reportsWithHeatmapData.length);
    }
  }, [reportsWithHeatmapData.length, filteredMapCount]);
  ...
  return (
    <div className="h-full bg-background overflow-y-auto">
      ...
      <EntityPageHeader
        title="Mapa de calor"
        description="Visualiza la concentración de reportes por zona"
        icon={MapPin}
        entityKey="rastreo"
        showCreate={false}
      ...
      <div className="flex-1 relative">
          ...
          <Card className="bg-background/90 backdrop-blur-sm shadow-md">
            <CardContent className="p-2">
              <p className="text-xs text-muted-foreground flex items-center gap-1">
                ...
                {loadingReportes ? 'Cargando...' : `${filteredMapCount} reportes`}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="h-full rounded-lg overflow-hidden border border-border">
          <LiveNavigationMap
            ...
            reports={reportsWithHeatmapData}
            nearestReportId={nearestReport?.id ?? null}
            isNavigationActive={isNavigationActive}
            onReportClick={handleReportClick}
            className="h-full"
            categories={categoriesForHeatmap}
            tipos={tiposForHeatmap}
            onFilteredCountChange={setFilteredMapCount}
          />
        </div>
      </div>
