...
export default function UsuariosPage() {
  ...
  // Hook de bulk actions escalable para profiles (+1000 registros)
  const bulkActions = useScalableBulkActions<User>(users, {
    tableName: 'profiles',
    queryKey: 'users',
    hasSoftDelete: true,
    statusColumn: 'estado',
  ...
  const handleOpenDelete = () => {
    dialog.open({
      ...
      onConfirm: async () => {
        try {
          // Usar la misma lógica que la eliminación individual (RPC admin_soft_delete_profile)
          const results = await Promise.allSettled(
            bulkActions.selectedIds.map((profileId) =>
              supabase.rpc('admin_soft_delete_profile', { p_profile_id: profileId })
            )
          );
          ...
          if (failCount > 0) {
            toast.error(`${failCount} usuario(s) no se pudieron eliminar (permisos/RLS)`);
          }
        } catch (error) {
          console.error('Error en eliminación masiva:', error);
          toast.error('Error: no se pudieron eliminar los usuarios seleccionados');
        }
      },
    });
  };
  ...
  const actions: BulkAction[] = [
    {
      ...
      variant: 'outline',
      requiredPermission: 'editar_usuario',
    },
    {
      id: 'change-roles',
      label: 'Agregar Rol',
    ...
    },
    {
      ...
      variant: 'destructive',
      requiredPermission: 'eliminar_usuario',
    },
  ];
