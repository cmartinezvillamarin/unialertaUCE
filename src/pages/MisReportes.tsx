...
export default function MisReportes() {
  ...
  // Obtener datos
  const { data: allReportes = [] } = useOptimizedReportes();
  ...
  // Filtrar solo los reportes creados por el usuario actual
  const reportes = useMemo(() => {
    if (!profile?.id) return [];
    return allReportes.filter(r => r.user_id === profile.id);
  }, [allReportes, profile?.id]);
  ...
  const handleCreate = () => {
    ...
    if (hasAdvancedRole) {
      navigate('/reportes/nuevo', { state: { backTo: '/mis-reportes' } });
    } else {
      navigate('/crear-reporte', { state: { backTo: '/mis-reportes' } });
    }
  };
  ...
  const handleOpenAssign = () => {
    ...
    dialog.open({
      ...
      onConfirm: async (selectedOption) => {
        ...
        // Enviar notificación PWA + in-app si se asignó a alguien
